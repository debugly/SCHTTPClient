...
static NSString* SCHTTPExecutorConvertToNSString(uint8_t* buffer, size_t length)
{
...
static BOOL SCHTTPExecutorIsFinalHeader(uint8_t* buffer, size_t size, size_t length)
{
...
static size_t SCHTTPExecutorReadStatusLine(uint8_t* buffer, size_t size, size_t length, SCHTTPRequestContext* context)
{
    NSString* line = SCHTTPExecutorConvertToNSString(buffer, length);
    SCHTTPEnsureSuccessOrReturn0([context beginResponseWithLine:line]);

    // Subsequent callbacks will hit SCHTTPExecutorReadHeader()
    return length;
}

static size_t SCHTTPExecutorReadHeader(uint8_t* buffer, size_t size, size_t length, SCHTTPRequestContext* context)
{
    BOOL endOfHeaders = SCHTTPExecutorIsFinalHeader(buffer, size, length);

    if (!endOfHeaders) {
        NSString* headerLine = SCHTTPExecutorConvertToNSString(buffer, length);
        SCHTTPEnsureSuccessOrReturn0([context addHeaderToCurrentResponse:headerLine]);

        // Subsequent callbacks will keep hitting SCHTTPExecutorReadHeader()
        return length;
    }

    // End of headers reached, data will follow
    SCHTTPLogTrace(@"%@ | All headers received.", context);
    BOOL canProceed = YES;
    if ([context isCurrentResponse100Continue]) {
        // Subsequent callbacks will hit SCHTTPExecutorReadStatusLine()
        [context finishCurrentResponse];
    } else {
        // Subsequent callbacks will hit SCHTTPExecutorAppendData()
    ...
    if ([context isUploadPaused]) {
        SCHTTPLogTrace(@"%@ | Response received (%lu) and upload was paused; unpausing...",
...
static size_t SCHTTPExecutorAppendData(uint8_t* buffer, size_t size, size_t length, SCHTTPRequestContext* context)
{
    SCHTTPEnsureSuccessOrReturn0([context appendDataToCurrentResponse:buffer withLength:length]);
...
static size_t SCHTTPExecutorSendCallback(uint8_t* buffer, size_t size, size_t length, SCHTTPRequestContext* context)
{
    ...
    if ([context hasUploadBeenAccepted]) {
        ...
        [context pauseUpload];
        SCHTTPLogTrace(@"%@ | ReadCallback: 100-Continue hasn't been received yet, holding off upload.", context);
        return CURL_READFUNC_PAUSE;
    }
}

static size_t SCHTTPExecutorReceiveCallback(uint8_t* buffer, size_t size, size_t length, SCHTTPRequestContext* context)
{
    ...
    switch (context.state) {
        ...
        case SCHTTPResponseStateReadingStatusLine:
            return SCHTTPExecutorReadStatusLine(buffer, size, length, context);

        case SCHTTPResponseStateReadingHeaders:
            return SCHTTPExecutorReadHeader(buffer, size, length, context);

        case SCHTTPResponseStateReadingData:
            return SCHTTPExecutorAppendData(buffer, size, length, context);
...
static int SCHTTPExecutorDebugCallback(CURL* handle, curl_infotype type, char* text, size_t length, void* context)
{
    switch (type) {
        case CURLINFO_TEXT: {
            NSString* message = [[NSString alloc] initWithBytes:text length:length encoding:NSASCIIStringEncoding];
            SCHTTPCurlDebug(@"%@", message);
            break;
        }

        case CURLINFO_DATA_IN:
            SCHTTPCurlDebug(@"DATA IN << %lub", (unsigned long)length);
            break;

        case CURLINFO_DATA_OUT:
            SCHTTPCurlDebug(@"DATA OUT >> %lub", (unsigned long)length);
            break;

        case CURLINFO_SSL_DATA_IN:
            SCHTTPCurlDebug(@"SSL DATA IN << %lub", (unsigned long)length);
            break;

        case CURLINFO_SSL_DATA_OUT:
            SCHTTPCurlDebug(@"SSL DATA OUT >> %lub", (unsigned long)length);
            break;
...
@interface SCHTTPExecutor ()

@property (strong) NSMutableArray* running;
@property (strong) dispatch_queue_t synchronizationQueue;
@end

@implementation SCHTTPExecutor
{
...
static BOOL SCHTTPExecutorInitialized = NO;
...
+ (void)initialize
{
    if (!SCHTTPExecutorInitialized) {
        CURLcode result = curl_global_init(CURL_GLOBAL_ALL);
        if (result != CURLE_OK) {
            SCHTTPLogError(@"curl_global_init() failed with code %d; some functionalities may be impaired.", result);
        } else {
            SCHTTPLogInfo(@"curl_global_init() successfully executed; SCHTTP booted with libcurl '%s'.",
                          LIBCURL_VERSION);
            SCHTTPExecutorInitialized = YES;
        }
    }
}
...
{
    SCHTTPSingleton(SCHTTPExecutor, instance, [[self alloc] initWithId:@"Shared"]);
...
- (BOOL)executeRequest:(SCHTTPRequest*)request
{
    ...
    dispatch_sync(_synchronizationQueue, ^{
        ...
        if ([self.running count] >= self.maxParallelRequests) {
...
+ (void)cleanup
{
    if (SCHTTPExecutorInitialized) {
        curl_global_cleanup();
        SCHTTPExecutorInitialized = NO;
        SCHTTPLogInfo(@"curl_global_cleanup() performed.");
    }
}
...
- (void)prepareContextForExecution:(SCHTTPRequestContext*)context
{
    SCHTTPRequest* request = context.request;

    if ((request.version == SCHTTPProtocolVersion_1_1) &&
        ...
        SCHTTPLogDebug(@"%@ | Adding 'Expect: 100-Continue' header to request (upload size > %lu)",
    ...
    if ([context is100ContinueRequired]) {
        ...
        // This part is just the setup, check out SCHTTPExecutorSendCallback() for the logic.
        [context waitFor100ContinueBeforeUploading];
    }

    if ([request isUpload] &&
        (request.chunkedTransfer || ![request isUploadSizeKnown])) {
        SCHTTPLogDebug(@"%@ | Upload size is unknown, adding 'Transfer-Encoding: chunked' header.", context);
        [request setValue:HV(Chunked) forHeader:H(TransferEncoding)];
    }
}

- (void)createContextAndExecuteRequest:(SCHTTPRequest*)request
{
    CURL* handle = [self getOrCreatePooledCurlHandle];
    SCHTTPRequestContext* context = [[SCHTTPRequestContext alloc] initWithRequest:request andCurlHandle:handle];
...
{
    while (true) {
        SCHTTPRequest* nextRequest = [self popQueuedRequest];
...
- (BOOL)isAlreadyRunningOrQueued:(SCHTTPRequest*)request
{
    return [_running containsObject:request] || [_queued containsObject:request];
}

- (void)enqueueRequest:(SCHTTPRequest*)request
{
    [_queued addObject:request];
}

- (void)addToRunning:(SCHTTPRequest*)request
{
    [_running addObject:request];
}

- (void)removeFromRunning:(SCHTTPRequest*)request
{
    [_running removeObject:request];
}

- (SCHTTPRequest*)popQueuedRequest
{
    if ([_queued count] == 0) return nil;

    SCHTTPRequest* request = _queued[0];
    [_queued removeObjectAtIndex:0];
    
    return request;
}

- (void)executeContext:(SCHTTPRequestContext*)context withCurlHandle:(CURL*)handle
{
    SCHTTPRequest* request = context.request;
...
//    curl_easy_setopt(handle, CURLOPT_FORBID_REUSE, 1L);
    if (_verbose) {
        ...
        curl_easy_setopt(handle, CURLOPT_DEBUGFUNCTION, SCHTTPExecutorDebugCallback);
    }

    // Setup - request line
    if (request.version == SCHTTPProtocolVersion_1_0) {
    ...
    if ([request isUpload]) {
        ...
        curl_easy_setopt(handle, CURLOPT_READFUNCTION, SCHTTPExecutorSendCallback);
    ...
    curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, SCHTTPExecutorReceiveCallback);
...
//    }
    ...
    SCHTTPLogInfo(@"%@ | Request starting…", context);
    ...
    if ([request wasCancelled]) {
        SCHTTPSingleton(NSError, cancelError, SCHTTPError(SCHTTPErrorCodeCancelled, @"Request cancelled."));
        [context requestFinishedWithError:cancelError];
        SCHTTPLogInfo(@"%@ | Request cancelled.", context);
        ...
        SCHTTPLogInfo(@"%@ | Request abnormally terminated:(%ld) %@", context,(long)[error code] ,[error localizedDescription]);

    } else {
        [context requestFinished];
        SCHTTPLogInfo(@"%@ | Request finished.", context);
    }
}
...
- (NSError*)convertCURLCodeToNSError:(CURLcode)code context:(SCHTTPRequestContext*)context
{
    ...
    if (reason == nil) return SCHTTPError(code, description);
    else return SCHTTPErrorWithReason(code, description, reason);
}
