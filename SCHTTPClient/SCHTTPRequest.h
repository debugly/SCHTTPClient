...
struct SCTransferSpeed {
    long bytesPerSecond;
    long duration;
};
typedef struct SCTransferSpeed SCTransferSpeed;



#pragma mark - Utility Functions

extern SCTransferSpeed SCTransferSpeedMake(NSUInteger bytesPerSecond, NSTimeInterval duration);
extern NSString* NSStringFromBBTransferSpeed(SCTransferSpeed transferSpeed);
...
/**
 The `SCHTTPRequest` class represents an HTTP or HTTPS request to a [resource in a remote server](<url>), using a 
 given <verb> and a [protocol version](<version>).

 It contains several properties which will influence how the request will be executed by the `<SCHTTPExecutor>` to which
 they are submitted.
 */
...
@property(strong, nonatomic) id<SCHTTPContentHandler> responseContentHandler;
 ...
 */
@property(assign, nonatomic) SCTransferSpeed downloadTimeout;
 ...
 */
@property(assign, nonatomic) long uploadSpeedLimit;
...
/**
 ...
 Defaults to 0 (unlimited speed).
 */
@property(assign, nonatomic) long downloadSpeedLimit;
...
@property(assign, nonatomic, readonly) SCHTTPProtocolVersion version;
...
@property(strong, nonatomic, readonly) SCHTTPResponse* response;
...
/**
 ...
 `<SCHTTPExecutor>` in which it will be run
 */
- (BOOL)cancel;
