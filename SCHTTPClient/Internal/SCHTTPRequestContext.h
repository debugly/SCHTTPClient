...
typedef NS_ENUM(NSUInteger, SCHTTPResponseState) {
    SCHTTPResponseStateReady = 0,
    SCHTTPResponseStateSendingData,
    SCHTTPResponseStateReadingStatusLine,
    SCHTTPResponseStateReadingHeaders,
    SCHTTPResponseStateReadingData,
    SCHTTPResponseStateFinished
};
...
/**
 The `SCHTTPRequestContext` class holds the originating request and all the responses received in its context.
 
 It mainly serves the purpose of hiding away some of the logic present in transitioning between states in the lifecycle
 of an HTTP request/response flow, as well as factoring out some of the logic that `<SCHTTPExecutor>` requires.
 As such, has no value outside of it.
 */
@interface SCHTTPRequestContext : NSObject
...
@property(strong, nonatomic, readonly) SCHTTPRequest* request;
@property(assign, nonatomic, readonly) CURL* handle;
@property(strong, nonatomic, readonly) NSError* error;
@property(strong, nonatomic, readonly) SCHTTPResponse* currentResponse;
@property(strong, nonatomic, readonly) SCHTTPResponse* lastResponse;
@property(assign, nonatomic, readonly) SCHTTPResponseState state;
