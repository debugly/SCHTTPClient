...
NSUInteger SCHTTPLogLevel = SCHTTPLogLevelTrace;

void SCHTTPLog(NSUInteger level, NSString* prefix, NSString* (^statement)(void))
{
    ...
    if (level <= SCHTTPLogLevel) NSLog(@"SCHTTP | %@ | %@", prefix, statement());
}
...
NSString* SCHTTPMimeType(NSString* file)
{
#ifndef __UTTYPE__
...
long long SCHTTPCurrentTimeMillis()
{
    struct timeval t;
    gettimeofday(&t, NULL);
...
NSString* SCHTTPURLEncode(NSString* string, NSStringEncoding encoding)
{
