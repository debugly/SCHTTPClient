...
@implementation SCHTTPSelectiveDiscarder
                     ...
                     error:(NSError**)error
{
    if (![self isAcceptableResponseCode:statusCode]) {
        if (error != NULL) {
            *error = SCHTTPErrorWithFormat(statusCode, @"Unnacceptable response: %lu %@",
    ...
    if (![self isAcceptableContentType:contentType]) {
        if (error != NULL) {
            *error = SCHTTPErrorWithFormat(SCHTTPErrorCodeUnnacceptableContentType,
...
{
    SCHTTPSingleton(SCHTTPSelectiveDiscarder, instance, [[SCHTTPSelectiveDiscarder alloc] init]);
    return instance;
}
