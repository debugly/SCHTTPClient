...
@interface SCHTTPFileWriter ()

@property (copy) NSString* pathToFile;
@property (strong) NSOutputStream* stream;;
@property (assign) BOOL needsCleanup;;
@end

@implementation SCHTTPFileWriter
...
#pragma mark SCHTTPContentHandler
...
- (void)deleteFileInBackground
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSError* error = nil;
        if (![[NSFileManager defaultManager] removeItemAtPath:self.pathToFile error:&error]) {
            ...
            SCHTTPLogWarn(@"[%@] Deletion of partially downloaded file '%@' failed: %@",
                          self, self.pathToFile, description);
        } else {
            SCHTTPLogTrace(@"[%@] Deleted partially downloaded file '%@'.", self, self.pathToFile);
        }
    });
}
