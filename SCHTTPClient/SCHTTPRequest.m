...
SCTransferSpeed SCTransferSpeedMake(NSUInteger bytesPerSecond, NSTimeInterval duration)
{
    SCTransferSpeed transferSpeed;
...
NSString* NSStringFromBBTransferSpeed(SCTransferSpeed transferSpeed)
{
    return [NSString stringWithFormat:@"%ld/s for %ld seconds",
            transferSpeed.bytesPerSecond, transferSpeed.duration];
}
...
{
    SCHTTPEnsureNotNil(url);
    SCHTTPEnsureNotNil(verb);
    ...
    if (self != nil) {
        ...
        _downloadTimeout = SCTransferSpeedMake(1024, 20);
        _uploadSpeedLimit = 0;
        ...
        SCHTTPSingleton(NSString, appName,
                        [[[[NSBundle mainBundle] infoDictionary] objectForKey:(NSString*)kCFBundleExecutableKey]
                         stringByReplacingOccurrencesOfString:@" " withString:@"-"]);
        SCHTTPSingleton(NSString, appVersion,
                        [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"]);

#if defined(__IPHONE_OS_VERSION_MIN_REQUIRED)
        SCHTTPSingletonString(userAgent, @"SCHTTP/%@ %@/%@ (%@; iOS %@; Scale/%0.2f)",
                              SCHTTPVersion, appName, appVersion,
                              [[UIDevice currentDevice] model],
                              [[UIDevice currentDevice] systemVersion],
                              [[UIScreen mainScreen] scale]);
#elif defined(__MAC_OS_X_VERSION_MIN_REQUIRED)
        SCHTTPSingletonString(userAgent, @"SCHTTP/%@ %@/%@ (Mac OS X %@)",
                              SCHTTPVersion, appName, appVersion,
...
- (double)downloadTransferRate
{
    ...
    long long end = (self.endTimestamp > 0 ? self.endTimestamp : SCHTTPCurrentTimeMillis());
...
{
    SCHTTPEnsureNotNil(stream);
    SCHTTPEnsureNotNil(contentType);

    if (_version != SCHTTPProtocolVersion_1_1) return NO;
...
{
    SCHTTPEnsureNotNil(path);

    NSError* err = nil;
    NSDictionary* attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:path error:&err];
    if (err != nil) {
        if (error != NULL) *error = err;
        SCHTTPLogError(@"Can't read file attributes: %@", [err localizedDescription]);
        return NO;
    }

    unsigned long long size = [attributes fileSize];
    if (size == 0) {
        if (error != NULL) *error = SCHTTPError(-1, @"File is empty (0 bytes)");
        SCHTTPLogError(@"File is empty (0 bytes)");
        return NO;
    } else if (size > NSUIntegerMax) {
        // This can probably be relaxed...
        SCHTTPLogError(@"File is too large (>%lu bytes)", NSUIntegerMax);
        if (error != NULL) *error = SCHTTPErrorWithFormat(-2, @"File is too large (>%lu bytes)", NSUIntegerMax);
        return NO;
    }

    [self setValue:SCHTTPMimeType(path) forHeader:H(ContentType)];
...
- (BOOL)setUploadFormData:(NSDictionary*)formData
{
    ...
    [formData enumerateKeysAndObjectsUsingBlock:^(NSString* key, id obj, BOOL* stop) {
        NSString* formEncodedKey = SCHTTPURLEncode(key, NSASCIIStringEncoding);
        NSString* formEncodedValue;
        if (obj == nil) {
            formEncodedValue = @"";
        } else {
            formEncodedValue = SCHTTPURLEncode(obj, NSASCIIStringEncoding);
        }
...
{
    SCHTTPEnsureNotNil(value);
    SCHTTPEnsureNotNil(header);
...
- (BOOL)cancel
{
    ...
    long long now = SCHTTPCurrentTimeMillis();
